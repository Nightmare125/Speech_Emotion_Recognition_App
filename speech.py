...
def main():
    ...
    if selected_box == 'Emotion Recognition':
        st.sidebar.success('Add an audio file .')
        application()
    if selected_box == 'View source code':
        st.code(get_file_content_as_string('speech.py'))


@st.cache(show_spinner=False)
def get_file_content_as_string(path):
    url = 'https://github.com/Nightmare125/S.E.R-app/main/' + path
...
@st.cache(show_spinner=False)
def load_model():
    model = tf.keras.models.load_model('speech.h5')
    return model
...
def application():
    ...
    if file_to_be_uploaded:
        ...
        st.success('Emotion in the speech is : ' + predict(model, file_to_be_uploaded))
